...
def generate_api_test_scenarios(ppc, system_data, n_samples,
    ...
    for i in range(min(n_api_buses, n_buses_sd)):
        api_P_full[i] = api_P[i] * base_scale
        api_Q_full[i] = api_Q[i] * base_scale
    ...
    for i in range(n_samples):
        var_i = float(variation) if variation is not None else float(
            np.random.choice(np.array(variation_levels, dtype=float)))
